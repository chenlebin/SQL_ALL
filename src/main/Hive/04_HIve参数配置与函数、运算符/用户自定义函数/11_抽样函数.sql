...
--todo TABLESAMPLE (BUCKET x OUT OF y [ON colname])
--todo 优点：速度快，
--1、y必须是table总bucket数的倍数或者因子。hive根据y的大小，决定抽样的比例。
--例如，table总共分了4份（4个bucket），总结：抽取数据就是分桶数/y
-- 因数： 当y=2时，抽取(4/2=)2个bucket的数据，
-- 倍数： 当y=8时，抽取(4/8=)1/2个bucket的数据。
--2、x表示从哪个bucket开始抽取。
--例如，table总bucket数为4，tablesample(bucket 4 out of 4)，表示总共抽取（4/4=）1个bucket的数据，抽取从第4个桶开始抽取，所以抽取第4个bucket的数据。
--注意：x的值必须小于等于y的值，否则FAILED:Numerator should not be bigger than denominator in sample clause for table stu_buck
--3、ON colname表示基于什么抽
--ON rand()表示随机抽
--ON 分桶字段 表示基于分桶字段抽样 todo 效率更高 推荐
...
--TABLESAMPLE (BUCKET x OUT OF y [ON rand()|colname])
