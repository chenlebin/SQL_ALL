...
--数据装载
insert overwrite table dwt_visitor_topic partition (dt = '2021-10-13')
select nvl(1d_ago.mid_id, old.mid_id),
       ...
       case
           when old.mid_id is null and 1d_ago.is_new = 1 then '2021-10-13'
           when old.mid_id is null and 1d_ago.is_new = 0 then '2020-06-13'--无法获取准确的首次登录日期，给定一个数仓搭建日之前的日期
           else old.visit_date_first end,
       if(1d_ago.mid_id is not null, '2021-10-13', old.visit_date_last),
...
from (
         ...
         where dt = date_add('2021-10-13', -1)
     ) old
         full outer join
     (
         select mid_id,
     ...
     (
         select mid_id,
                ...
                visit_count
         from dws_visitor_action_daycount
         where dt = date_add('2021-10-13', -7)
     ) 7d_ago
